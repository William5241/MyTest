...
#warning 字体错误
        alert.iconLabel.font =  [UIFont systemFontOfSize:13]; //Font_ZHUNHei(13);
...
-(CGSize)hudSize
{
    if(CGSizeEqualToSize(self.size, CGSizeZero))
        return CGSizeMake(PHONE_WIDTH-80, 50);
    return self.size;
}
...
-(UIFont*)bodyFont
{
    if(_bodyFont)
        return _bodyFont;
    _bodyFont = [UIFont systemFontThatFitsSize:[self labelConstraint] maxFontSize:22 minSize:20 text:self.bodyText];
    return _bodyFont;
    ...
    if(_bodyFont)
        return _bodyFont;
    float size = 0;
    [self.bodyText sizeWithFont:[UIFont boldSystemFontOfSize:20] minFontSize:12 actualFontSize:&size forWidth:self.contentRect.size.width / 1.3 lineBreakMode:NSLineBreakByTruncatingTail];
